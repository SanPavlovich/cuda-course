...
int main()
{
    ...
    nvtxRangePush("Memory Copy Allocation");
    ...
    // Run the CPU version.
    nvtxRangePush("Kernel Execution CPU");
    ...
    nvtxRangePush("Memory copy from CPU to GPU");
    ...
    // Launch the kernel.
    nvtxRangePush("Kernel Execution GPU");
    ...
    // Copy C from the device.
    nvtxRangePush("Memory copy from GPU to CPU");
    CHECK_CUDA_ERROR(cudaMemcpy(C_host_from_device, C_device,
                                m * ldc * sizeof(float),
                                cudaMemcpyDeviceToHost));
    nvtxRangePop();
